...
rule bayesspace:
    ...
    params:
        d=data_directory + "/{sample}/outs/"
    shell:
        "workflow/scripts/sp-bayesspace.R  --sampleid {wildcards.sample} --pca.dimension {bayesspace_pca_dimension} --data.dir {params.d} --output.sce {output.sce} --output.enhanced {output.enhanced} --cluster.plot {output.clusterplot} --qplot {output.qplot}"
